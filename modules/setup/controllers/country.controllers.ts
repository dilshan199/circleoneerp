...
export class CountryControllers {
    ...
    static async store(request: NextRequest) {
        try {
            ...
            const country = await CountryServices.storeServices(body.country);
            return NextResponse.json(
                { success: true, message: "Country created successfully!", country },
                { status: 200 }
            );
        }
        catch (error) {
    ...
    static async show(country_id: number) {
        try {
            const country = await CountryServices.showServices(country_id);
            return NextResponse.json(
                { success: true, message: "Country fetched successfully!", data: country },
                { status: 200 }
            );
        }
        catch (error) {
    ...
    // PUT: Update country
    static async update(country_id: number, request: NextRequest) {
        try {
            const body = await request.json();
            await CountryServices.updateServices({ ...body.country, country_id });
            return NextResponse.json(
    ...
    static async delete(country_id: number) {
        try {
            await CountryServices.deleteServices(country_id);
            return NextResponse.json(
